...
class _DriverEditProfileScreenState extends State<DriverEditProfileScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (_driverId == null) {
      ...
      _driverId = args?['driver_id'] ?? args?['id'];
  ...
  Future<void> _fetchDriverData() async {
    try {
      final response = await http.get(
        Uri.parse('${ApiService.baseUrl}/api/profile?role=driver&id=$_driverId'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _avatarUrl = data['profile_pic']; // Fixed key to match driver_profile.dart
          _isLoading = false;
        });
      } else {
  ...
  Future<void> _pickImage(ImageSource source) async {
    try {
      ...
      _showError("Could not access ${source == ImageSource.camera ? 'camera' : 'gallery'}. Please check permissions.");
    }
  }
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('${ApiService.baseUrl}/api/driver/update-profile'),
      );
