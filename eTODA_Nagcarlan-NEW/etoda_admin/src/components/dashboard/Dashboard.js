      ...
      {loading ? <Loading/> : (
        <>
          ...
          <div className="metrics">
            {CARDS.map((c,i) => (
              <div key={i} className={`metric ${c.lbl === 'Open Complaints' && c.val > 0 ? 'animate-pulse border-red-500' : ''}`} style={{borderLeftColor:c.color}}>
                <div className="metric-val">{c.val}</div>
                <div className="metric-lbl">{c.lbl}</div>
                <div className="metric-sub">{c.sub}</div>
                <div className="metric-trend" style={{color: c.trendColor === 'green' ? 'green' : 'red', fontSize: '0.75rem'}}>{c.trend}</div>
                {c.lbl === 'Open Complaints' && c.val > 0 && <a href="/complaints" className="text-red-500 text-sm">Review Now →</a>}
              </div>
            ))}
          </div>
          <div className="quick-actions" style={{display:'flex', gap:12, margin:'20px 0'}}>
            <button className="btn btn-outline" style={{borderColor:'#1a4731', color:'#1a4731'}} onClick={() => window.location.href='/drivers'}>+ Enroll Driver</button>
            <button className="btn btn-outline" style={{borderColor:'#1a4731', color:'#1a4731'}} onClick={() => window.location.href='/fare'}>Configure Fare</button>
            <button className="btn btn-outline" style={{borderColor:'#1a4731', color:'#1a4731'}} onClick={() => window.location.href='/complaints'}>Review Complaints</button>
            <button className="btn btn-outline" style={{borderColor:'#1a4731', color:'#1a4731'}} onClick={() => notify('Report generated', 'success')}>Generate Report</button>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1.4fr 1fr',gap:18}}>
            <div className="card">
              ...
              <div style={{padding:'14px 18px'}}>
                {[['Go Backend API','Online'],['PostgreSQL Database','Connected'],['AES-256 QR Encryption','Active'],['Firebase FCM (Push Notifications)','Synced'],['Audit Trail Logging','Enabled']].map(([l,v]) => (
