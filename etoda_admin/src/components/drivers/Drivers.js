...
function Drivers({ notify }) {
  ...
  const BLANK = { username: "", password: "", name: "", franchise: "", body_no: "", contact: "", license_no: "" };
  ...
  const openQR = async (d) => {
    if (!d.qr_id) return;
    setQrSelected({ qr_id: d.qr_id, driver_name: capitalizeName(d.name), franchise: d.franchise, issued_at: d.created_at || "—" });
    try {
  ...
  const doCopyPw = () => {
    ...
    safeCopy(form.password).then(() => { setCopiedPw(true); setTimeout(() => setCopiedPw(false), 2000); })
      .catch(() => notify("Copy failed — try manually selecting the text", "error"));
  };
  const doCopyCreds = () => {
    if (!form.username || !form.password) return;
    safeCopy(`Username: ${form.username}\nPassword: ${form.password}`).then(() => { setCopiedCreds(true); setTimeout(() => setCopiedCreds(false), 2000); })
      .catch(() => notify("Copy failed — try manually selecting the text", "error"));
  };
  const handleNameBlur = (val) => { if (!usernameTouched && val.trim()) setForm(p => ({ ...p, username: suggestUsername(val) })); };

  // ── CRUD ──
  const enroll = async () => {
    if (!form.name.trim() || !form.franchise.trim()) { notify("Name and Franchise are required", "error"); return; }
    setSaving(true);
    const payload = { ...form, name: capitalizeName(form.name) };
    if (!payload.password) delete payload.password;
    const r = await api("/api/drivers", "POST", payload);
    setSaving(false);
    if (r.success) { setEnrollOpen(false); setForm(BLANK); setUsernameTouched(false); setShowPw(false); load(search); notify(`${capitalizeName(form.name)} enrolled`); }
  ...
  const saveEdit = async () => {
    setSaving(true);
    const payload = { ...form, name: capitalizeName(form.name) };
    if (!resetPwMode || !payload.password || !payload.password.trim()) delete payload.password;
  ...
  const openEdit = (d) => {
    ...
    setEditItem(d);
    setForm({ username: d.username || "", password: "", name: d.name, franchise: d.franchise, body_no: d.body_no || "", contact: d.contact || "", license_no: d.license_no || "" });
  };
  ...
  const confirmSuspend = async () => {
    ...
    if (r.success) { load(search); notify(next === "Inactive" ? `${capitalizeName(suspendItem.name)} has been suspended` : `${capitalizeName(suspendItem.name)} has been restored`, next === "Active" ? "success" : "warn"); }
  };
  ...
  const credentialSection = (isEdit = false) => {
    const showResetBlock = isEdit ? resetPwMode : true;
    return (
      <div style={{ borderTop: "1.5px solid var(--gray2)", marginTop: 4, marginBottom: 0, paddingTop: 14, paddingBottom: 4 }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input value={form.username} onChange={e => { setUsernameTouched(true); setForm(p => ({ ...p, username: e.target.value })); }} onBlur={() => { if (!isEdit) handleNameBlur(form.name); }} placeholder="e.g. juan.delacruz" style={{ width: "100%", boxSizing: "border-box" }} />
          {!isEdit && !usernameTouched && form.name && (
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              <div style={{ position: "relative", flex: 1 }}>
                <input type={showPw ? "text" : "password"} value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} placeholder={isEdit ? "Enter or generate a new password…" : "Enter or generate a temporary password…"} style={{ width: "100%", boxSizing: "border-box", paddingRight: 36, fontFamily: form.password && !showPw ? "monospace" : "inherit" }} />
  ...
  const profileFields = () => (
    <>
      ...
      <div className="form-row">
        <div className="field"><label>Full Name *</label><input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} onBlur={e => handleNameBlur(e.target.value)} placeholder="Juan A. Dela Cruz" /></div>
    ...
    <div>
      ...
      <div className="status-row">
        {[
          ...
          [inactiveCount, "Suspended",      "var(--red)",   "Inactive" ],
          [thisMonth,     "New This Month", "var(--blue)",  null       ],
        ].map(([v, l, c, filterVal]) => (
          ...
          <>
            <div className="tbl-wrap">
              <table>
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={10} style={{ textAlign: "center", padding: "24px", color: "var(--gray)" }}>No {statusFilter !== "All" ? statusFilter.toLowerCase() : ""} drivers found.</td></tr>
                  ) : paginated.map(d => (
        ...
        <Modal title={suspendItem.status === "Active" ? "Suspend Driver" : "Restore Driver"} onClose={() => setSuspendItem(null)}>
          {suspendItem.status === "Active" ? (
            <div style={{ background: "#fff8e1", border: "1px solid #f59e0b", borderRadius: 10, padding: "14px 16px", marginBottom: 16, fontSize: ".88rem", color: "#78350f", lineHeight: 1.7 }}>
              ...
              Their account will be <strong>deactivated</strong> and they won't be able to log in until restored.<br />
            ...
            <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 10, padding: "14px 16px", marginBottom: 16, fontSize: ".88rem", color: "#14532d", lineHeight: 1.7 }}>
              Restore <strong>{capitalizeName(suspendItem.name)}</strong> ({suspendItem.franchise})?<br />
