...
import { faPrint, faDownload, faTimes, faQrcode } from '@fortawesome/free-solid-svg-icons';
          ...
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          @media print {
            * {
            ...
            .sticker { border: 2px solid #2d5a1b !important; }
            .sticker-header {
              background: #2d5a1b !important;
            }
            .sticker-header h1,
      ...
      <body>
        <div class="sticker">
          ...
          <div class="sticker-footer">
            Nagcarlan LGU &middot; eTODA System &middot; Issued ${selected.issued_at}
          </div>
        </div>
        <script>
  ...
  return (
    <div>
                  ...
                  <tr key={q.id}>
                    ...
                    <td>
                      <div className="row-actions">
                        {q.status === 'Active' && (
                          <button className="ib ib-del" onClick={() => update(q.id, 'Revoked')}>Revoke</button>
                        )}
                        {q.status !== 'Active' && (
                          <button className="ib ib-edit" onClick={() => update(q.id, 'Active')}>Restore & Regen</button>
                        )}
                        <button
      ...
      {selected && (
        <div
          onClick={closeModal}
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '24px' }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ background: '#fff', borderRadius: '20px', width: '320px', overflow: 'hidden', boxShadow: '0 24px 60px rgba(0,0,0,0.2)' }}
          >
            {/* Header */}
            ...
            <div style={{ padding: '24px 20px', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                <button
                  ...
                  style={{ flex: 1, padding: '11px', fontSize: '13px', fontWeight: '700', borderRadius: '10px', cursor: 'pointer', background: '#2d5a1b', color: '#fff', border: 'none' }}
                >
                ...
                <button
                  onClick={downloadQR}
                  style={{ flex: 1, padding: '11px', fontSize: '13px', fontWeight: '600', borderRadius: '10px', cursor: 'pointer', background: '#f5f5f5', color: '#333', border: '1px solid #e0e0e0' }}
                >
              ...
              <button
                onClick={closeModal}
                style={{ width: '100%', padding: '10px', fontSize: '13px', fontWeight: '700', borderRadius: '10px', cursor: 'pointer', background: '#fee2e2', color: '#dc2626', border: '1px solid #fca5a5' }}
              >
