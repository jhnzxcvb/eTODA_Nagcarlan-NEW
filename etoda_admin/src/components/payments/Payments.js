          ...
          <div className="card-actions">
            <div style={{ position: 'relative' }}>
              ...
              <input
                ...
                placeholder="Search ref, passenger, driver..."
                value={search}
        ...
        <div style={{ padding: '10px 18px 0', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {['All', 'Pending', 'Settled', 'Refunded'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)} style={{
              ...
              {s === 'All' ? `All (${data.length})` : `${s} (${data.filter(p => p.status === s).length})`}
            </button>
          ))}
                ...
                <tbody>
                  {paginated.length === 0 ? (
                    ...
                    <tr key={p.id}>
                      ...
                      <td style={{ fontSize: '.85rem' }}><MethodIcon method={p.method} />{p.method}</td>
            ...
            {filtered.length > pageSize && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 18px', borderTop: '1px solid var(--gray2)' }}>
                <span style={{ fontSize: '.8rem', color: 'var(--gray)' }}>
                  Showing {Math.min((currentPage - 1) * pageSize + 1, filtered.length)}–{Math.min(currentPage * pageSize, filtered.length)} of {filtered.length} transactions
                </span>
